...
- Ref tables: `ref_tribunaux`, `ref_ste_adresses`, `ref_nationalites`, `ref_lieux_naissance`, `ref_activites_statuts`, `ref_activites_ompic`, `ref_formes_juridiques`
...
- **Configuration** (`configuration.php`): hub de groupes (onglet `?g=`) avec une page par référentiel (formes-juridiques, villes, tribunaux, nationalites, lieux-naissance, adresses, qualites-associe, fonctions, activites_statuts, activites-ompic). Add/edit/delete inline. L'onglet `formes-juridiques` affiche une colonne **Dossier Templates** pour lier chaque forme juridique à un dossier dans `templates/`. Si le dossier n'existe pas, il est créé automatiquement lors de l'ajout ou la modification.
...
- **Collaborateur en charge du dossier (étape 1, obligatoire)** : `<select name="societe_collaborateur_id" required>` alimenté par `fetch_collaborateurs_options($pdo)` (actifs uniquement, libellé `nom_complet — qualification / code`). Bouton adjacent `[data-quick-create-btn="collaborateurs"]` (droit `collaborateurs.create`) ouvrant `includes/collaborateur_quick_create_modal.php` avec `$quickCreateTargetSelect = 'societe_collaborateur_id'` → le collaborateur créé est ajouté et sélectionné sans rechargement. Double validation : `required` côté navigateur + refus côté serveur (ID absent **ou introuvable en base**) qui renvoie vers l'étape 1 avec flash, afin qu'un ID forgé n'atteigne jamais `collaborateur_societes` (étape 6).
...
## Activités OMPIC — NMA 2010 (`ref_activites_ompic`) vs Activités Statuts (`ref_activites_statuts`)
- **Deux listes distinctes, aucun recouvrement** :
  - `ref_activites_ompic` = **649 ACTIVITÉS de la NMA 2010** (codes 4 chiffres), utilisée **uniquement pour les dossiers de domiciliation** (champ « Activité certificat négatif »). Les 21 sections (A→U), 88 branches et 274 sous-branches ne sont **pas** stockées. Chargée par `database/migrations/20260926_000008_activites_nma2010_activites_seules.sql` (prune `code REGEXP '^[A-U]$' OR CHAR_LENGTH(code) IN (2,3)` + renormalisation `sort_order` 1→649 → idempotente) ainsi que par `database/seed.sql` et `database/import.sql`.
  - `ref_activites_statuts` = **liste libre** des Activités (Statuts) des dossiers **Création / Cession / PV AGO** (20 libellés de référence dans `seed.sql`/`import.sql`, modifiables par l'utilisateur). Table renommée depuis `ref_activites` par `database/migrations/20260926_000009_activites_statuts_renommage.sql` (renommage conditionnel + resynchronisation sur la liste de référence).
- **La NMA n'est pas modifiable depuis les formulaires** : ni bouton ni modale « Nouvelle activité OMPIC ». Seuls les changements NMA passent par l'onglet `activites-ompic` de la configuration. La modale « Nouvelle activité (Statuts) » (bouton `data-add-activite-ref` → POST `add_activite_ref` → `ref_activites_statuts`) reste disponible sur le wizard Création, la fiche société en mode `&edit=1` et le wizard Cession.
- `sort_order` = ordre canonique NMA (par section A→U, puis par code) pour `ref_activites_ompic`.
- Lecture : `fetch_activites_ompic_options(?PDO)` → `[{code, libelle}]` ; libellé pour affichage : `fetch_activites_ompic_display(?PDO, string $code)` (retourne la valeur brute si le code est introuvable). Liste Statuts : `fetch_reference_options(?PDO, 'ref_activites_statuts', 'activite')`.
- Recherche : module IIFE en fin de `assets/js/app.js`, **par délégation d'événements** (`input`/`search`/`change` capturés sur `document`) pour couvrir les lignes ajoutées dynamiquement. Insensible aux accents/majuscules (`normalize('NFD')`), remet le compteur à zéro filtre après un `change`.
- Le select est résolu **par conteneur** (`.ompic-picker` ou ligne `data-activite-item`) et non par id → aucun id dupliqué sur les lignes Statuts clonées.
- Rendu du champ « Activité certificat négatif » (`.ompic-picker` + compteur) : `creation_steps/step_01_Societe.php` et `societe_details.php` (mode `&edit=1`).
- Rendu du champ « Activités (Statuts) » : un `<input type="search" data-ompic-filter class="activite-search">` devant chaque `<select name="societe_activites_statuts[]">`, y compris dans les `<template data-activite-template>` — dans `creation_steps/step_01_Societe.php`, `societe_details.php` et `cession/cession_steps/step_01_Societe.php`. Toute valeur historique absente de la liste reste affichée grâce à l'<option selected>` de repli.
- Onglets Configuration : `activites_statuts` (`ref_activites_statuts`, page `pages/configuration/activites_statuts.php`) et `activites-ompic` (`ref_activites_ompic`).
...
- **Attribut `data-confirm`** = message d'avertissement. Rend une fenêtre unique dans le layout (`includes/confirm_dialog.php` via `includes/pied_page.php`) ; le module IIFE en fin de `assets/js/app.js` l'ouvre **par délégation** (`click` sur `a`/`button`, `submit` sur `form` en phase capture).
- Attributs optionnels : `data-confirm-title`, `data-confirm-ok`, `data-confirm-cancel`, `data-confirm-tone` (`danger` par défaut, `primary` pour une confirmation non destructive).
- **Rejeu de l'action** : à la confirmation, l'attribut est retiré le temps de `trigger.click()` (ou `form.requestSubmit()`) — le `name`/`value` du bouton submitteur et la validation HTML sont préservés, sans double interception.
- Fermeture : bouton d'annulation, bouton ✕, clic sur l'overlay, touche `Échap`. `aria-hidden` bascule, le focus revient sur le déclencheur.
- **Wizard Création** (`pages/dossiers/creation_steps/_main.php`) : le message du bouton *Annuler* dépend du type de dossier (« d'une société » / « d'une domiciliation ») et se met à jour dans le gestionnaire `change` de `[data-type-gen]`. Le flash après annulation (`_init.php`) reprend le même libellé.
